...
GENERATOR_DIR = ROOT_DIR / "generators" / "classical" / "hiking_binary"
...
CONFIGS = {
    "train": [
        (1 + index % 3, 2 + index % 3, 2 + index % 3, 101 + index)
        for index in range(NUM_INSTANCES_PER_SPLIT)
    ],
    "valid": [
        (4 + index % 3, 5 + index % 3, 5 + index % 3, 201 + index)
        for index in range(NUM_INSTANCES_PER_SPLIT)
    ],
    "test": [
        (7 + index % 3, 8 + index % 3, 8 + index % 3, 301 + index)
        for index in range(NUM_INSTANCES_PER_SPLIT)
    ],
}
