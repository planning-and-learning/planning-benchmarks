...
GENERATOR_DIR = ROOT_DIR / "generators" / "classical" / "delivery"
sys.path.insert(0, str(ROOT_DIR))

from generators.classical.delivery import make_problem  # noqa: E402
...
STRUCTURAL_SPACES = {
    "train": cartesian(range(2, 6), range(1, 5)),
    "valid": cartesian(range(6, 9), range(5, 8)),
    "test": cartesian(range(9, 14), range(8, 13)),
}
...
def main() -> int:
    ...
    for split, configs in CONFIGS.items():
        ...
        for index, (grid_size, num_packages, seed) in enumerate(configs, start=1):
            problem_path = split_dir / f"{split}-{index}.pddl"
            problem_path.write_text(
                make_problem(grid_size, num_packages, seed),
                encoding="utf-8",
            )
