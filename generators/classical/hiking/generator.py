...
def make_problem(
    ...
    for i in range(num_couples):
        ...
        init_facts.extend(
            [
                f"    (partners couple{i} guy{i} girl{i})",
    ...
    return f"""(define (problem hiking-{num_couples}-{num_cars}-{num_places})
  (:domain hiking)
  (:objects
    {" ".join(cars)} - car
    {" ".join(tents)} - tent
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Generate a Hiking PDDL problem.")
